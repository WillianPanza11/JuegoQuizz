...
export class ListPreguntasComponent implements OnInit {
  ...
  finalizarCuestionario(){
    ...
    //this.toastr.success('Registrado con exito', 'Guardado')
    //this.router.navigate(['/dashboard'])
    console.log(cuestionario);
    this.loading = true; 
    this._quizzService.crearCustionario(cuestionario).then(data=>{
      this.toastr.success('El custionario fue registrado con exito!', 'Registrado')
      this.router.navigate(['/dashboard'])
    }).catch(error =>{
      this.loading = false; 
      console.log(error);
  ...
  generarCodigo(){//https://www.npmjs.com/package/nanoid
